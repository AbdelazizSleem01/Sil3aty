  ...
  callbacks: {
    async jwt({ token, user, trigger }) {
      try {
        ...
        // Return empty token if JWT decryption fails to prevent session errors
        return {};
      }
    },
    async session({ session, token }) {
      try {
        ...
        // Return session without user data if there's an error
        return session;
      }
    },
  },
  ...
  logger: {
    error(code, metadata) {
      const isDecryptionFailed =
        ...
        (metadata?.message?.includes("decryption operation failed") ||
          metadata?.error?.message?.includes("decryption operation failed") ||
          String(metadata).includes("decryption operation failed") ||
          (metadata &&
            Object.values(metadata).some((val) =>
              String(val).includes("decryption operation failed")
            )));

      if (isDecryptionFailed) {
        console.warn(
          "⚠️ NextAuth session decryption failed. This is common if your NEXTAUTH_SECRET changed or you have stale cookies. Clear your browser cookies for localhost:3000 to fix."
        );
        return;
      }
